...
typeset -A ORB_CONFIG=(
  # Ресурсы VM. Потолки, а не резервирование: память OrbStack берёт по мере
  # надобности и возвращает macOS — при двух работающих контейнерах VM держала
  # 130 МиБ из шести доступных гигабайт. Смысл потолка в другом: на машине с
  # 8 ГиБ разовый тяжёлый контейнер, забравший 6 ГиБ, оставляет системе два и
  # уводит её в своп. Половина ОЗУ виртуалке, два ядра из восьми — системе,
  # чтобы под полной нагрузкой контейнеров интерфейс отвечал.
  cpu                        6
  memory_mib                 4096
