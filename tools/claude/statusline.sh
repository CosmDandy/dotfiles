...
# Момент времени как "8:40pm". Минуты обязательны: это дедлайн, до которого надо
...
# date (Linux) читают epoch разными флагами, локаль фиксируем — иначе %p может
# оказаться пустым. Регистр правим подстановкой, а не tr: ${x,,} требует bash 4.
fmt_clock() {
  local ts=$1 s
  s=$(LC_ALL=C date -r "$ts" +'%-I:%M%p' 2>/dev/null) \
    || s=$(LC_ALL=C date -d "@$ts" +'%-I:%M%p' 2>/dev/null)
  s=${s/AM/am}
  printf '%s' "${s/PM/pm}"
}
