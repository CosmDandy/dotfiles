...
# Час слишком короткое плечо на фоне недели: экстраполяция усиливает его ×168,
# и любой всплеск в начале окна рисует отклонение в разы больше правды.
WEEK_MIN_ELAPSED=43200  # первые полдня окна темп не считаем, выборка ещё шумит
...
EOF2
    ...
    if [ "$five_pct" -ge "$FIVEH_SHOW" ] || [ -n "$DEBUG_ALL" ]; then
      ...
      if [ "$five_pct" -ge 100 ]; then
        ...
        # решать нечего (запас больше DEV_SHOW_SEC), там голый reset. Стрелка
        # добавляется только при отрицательном отклонении: reset+dev тогда —
        # момент РАНЬШЕ реального сброса, когда упрёмся в 0% при этом темпе.
        # При положительном dev reset+dev уехал бы ПОЗЖЕ настоящего сброса —
        # окно откроется раньше, чем истощится, и такое время бессмысленно.
  ...
  if [ -n "$week_pct" ] && [ "$want_week" -eq 1 ]; then
    # Отклонение недели — не экстраполяция темпа (то деление на pct раньше
    # улетало в разы при малом проценте или коротком плече: +53д, -3.8д на
    # ровном месте), а сравнение факта с линейным графиком. «Идеальный»
    # процент на этот момент = прошло_времени/неделя*100; dev — это прошедшее
    # время минус время, которое представляет фактический процент.
    # Запас (недобор) сюда не идёт: цель — выбрать подписку целиком, а не
    # экономить, так что плюс никого не интересует. Любое отставание от
    # графика показываем числом; красным — только когда отставание заметное
    # (больше WEEK_FLAT_SEC), некритичное — нейтральным цветом.
    color="$DIM"
    week_dev=""
