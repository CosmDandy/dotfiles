...
- [x] **Закрыто 2026-07-26.** Вопрос был: грузятся ли user-scope правила безусловно, независимо от `paths:`. Проверено эмпирически — `claude -p` в пустом каталоге вне репозитория, вопрос «какие rules-файлы сейчас в контексте» → ответ `NONE`. Значит `paths:` действительно скоупит, а замеченное ранее появление `security.md` объяснялось тем, что на тот момент у файла не было frontmatter. Расчёт бюджета в части 4 верен: path-scoped правило стоит ноль до срабатывания.
