...
# NOTE: the whole script is interactive (open url/app, "press Enter") — headless, every
# open fails with an RBS launch error. Run it by hand in a GUI session.
if [[ ! -t 0 ]]; then
    print_section "Skipping interactive app setup (no tty) — запусти platform/macos/install-extra.sh вручную"
    exit 0
fi

# Distribution sources live in the private submodule, because this repository is public.
# NOTE: sourced rather than run as a separate process — it needs print_section, confirm and
# setup_app from common.sh. The file only DECLARES functions; they are called below.
...
# NOTE: activation stays exactly here — the apps the tool patches are installed and
...
# NOTE: the BetterDisplay plist is not kept in the repo (EDID, monitor serials, licence
# marker in a public repository). Settings are restored from the restic backup:
#   restic restore --target / --include "$HOME/Library/Preferences/pro.betterdisplay.BetterDisplay.plist" latest
# then restart BetterDisplay. Displays are bound by hardware UUID, so on a fresh macOS the
# display may need relinking.
