...
- `--build-on-remote` обязателен: мак — aarch64-darwin, x86_64-linux локально не
  собирается.
- Память на время установки. Store инсталлятора — tmpfs в половину RAM, а
  замыкание системы с профилем devops это ~7 ГБ. На 16 ГБ получается 8 ГБ store,
  впритык; на 48 ГБ — 24 ГБ, с запасом. После установки VM живёт на 16 ГБ.
